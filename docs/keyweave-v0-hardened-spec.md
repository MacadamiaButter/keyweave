...
Provenance: a design-hardening pass on 2026-08-08 running five independent design lanes against
five adversarial verification lanes, each design attacked by a reviewer that did not write it.
This document is the synthesis, produced by reading the findings directly rather than the
reviewers' own summaries of them.
...
7. **[CRITICAL] The reproducible-build "honesty keystone" has an in-band anchor and no closed
   ...
   release on the public repo (GitHub is already in the story, not served by the VPS); document a
   received-bytes verification procedure (DevTools → sha256 the exact bytes your browser got →
   compare the tag) and state that only the victim's own browser can run it; drop any in-app
   "verify hash" affordance (self-attestation: malicious code reports the honest hash).
...
solutions. The crypto layer is buildable from THIS spec (not the plan sketch). Next step is the
Build the crypto and relay core (offline, testable, no attended step), fixes baked in, with
independent adversarial verification at each checkpoint. Public deploy remains
gated on the build-host condition (clean-room artifact plus out-of-band hashes, or a build host
with no unattended path to root).
...
domains, and builders separated from verifiers throughout. Final state: **client 81/81 tests, relay 75/75,
both green with typecheck clean**, verified by the conductor (not just the agents).
...
(owner-run, gated on the H1 override condition). The durable-state wiring that connects the client
