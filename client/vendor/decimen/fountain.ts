...
import { MAX_BLOCK_LEN, MAX_K, MAX_TOTAL_LEN, splitmix32 } from "./frame";
...
export class LTDecoder {
  ...
  constructor(
    ...
    // KEYWEAVE HARDENING (not upstream): bound the CLASS, not only the paths that
    // reach it. Upstream issue #1's proof of concept is a direct construction
    // (k=1, blockLen=8, totalLen=256MB makes assemble() zero-fill 256 MB). parseFrame()
    // and headerRefusal() both reject that header, but neither of them runs on
    // `new LTDecoder(...)`, and this directory is importable by any future caller, so
    // the "still bounded" claim has to be enforced here to be true.
    //
    // Ceilings only. Whether (k-1)*blockLen < totalLen <= k*blockLen holds is a question
    // about a header, not about a decoder, and stays with the two frame walls. This is
    // unreachable from OpticalReceiver, which enforces the identical three ceilings
    // before it constructs anything.
    if (!Number.isInteger(k) || k < 1 || k > MAX_K) {
      throw new RangeError(`LTDecoder: k out of range: ${k}`);
    }
    if (!Number.isInteger(blockLen) || blockLen < 1 || blockLen > MAX_BLOCK_LEN) {
      throw new RangeError(`LTDecoder: blockLen out of range: ${blockLen}`);
    }
    if (!Number.isInteger(totalLen) || totalLen < 1 || totalLen > MAX_TOTAL_LEN) {
      throw new RangeError(`LTDecoder: totalLen out of range: ${totalLen}`);
    }
    this.words = Math.ceil(blockLen / 4);
